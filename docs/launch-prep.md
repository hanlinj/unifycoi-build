...
### SEC-18 — `requestPasswordReset` emits no audit event *(discovered Phase 12 Slices 4a/5a/5a.1, carried again through Slice 6, still open)*
...
- **Source:** Flagged repeatedly across Phase 12 Slices 4a, 5a, and 5a.1 as a carried-forward ledger item; never banked into this document until now, and never fixed (each slice confirmed it was still true and moved on, since fixing it wasn't in scope for any of those slices). Re-confirmed still true and re-flagged explicitly at the close of Slice 6 (tenant cockpit) — small, pre-existing, worth closing before go-live rather than letting it carry indefinitely.
